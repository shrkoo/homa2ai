 ...
 * If ZARINPAL_MERCHANT_ID is not set: returns { error: 'NO_PAYMENT_PROVIDER' }
 * NEVER fakes a successful payment. NEVER returns success without gateway verification.
 *
 * Flow:
 *   1. createPayment → { authority, redirect_url } or { error }
 *   2. verifyPayment → { ok, ref_id } or { error: 'PAYMENT_FAILED' }
 *   3. completeOrder → atomic: update order + grant credits (idempotent via credits_added flag)
 */
...
async function zarinpalCreate(env, { amount, description, callback_url }) {
  ...
  try {
    const res = await fetch(ZARINPAL_API, {
      ...
      body: JSON.stringify({ merchant_id: merchantId, amount, description: description || 'Homa AI Credits', callback_url }),
    });
    const data = await res.json();
    const authority = data?.data?.authority;
    if (!authority) return { error: ErrorCodes.PAYMENT_FAILED, detail: data?.errors?.message || 'No authority' };
...
async function zarinpalVerify(env, { authority, amount }) {
  ...
  try {
    ...
    if (code === 100 || code === 101) return { ok: true, ref_id: String(refId || code) };
...
export const payments = {
  isAvailable(env) { return !!env.ZARINPAL_MERCHANT_ID; },
  async createPayment(env, opts) { return await zarinpalCreate(env, opts); },
  async verifyPayment(env, opts) { return await zarinpalVerify(env, opts); },
  ...
  // Idempotent: credits_added flag prevents double-grant on callback replay
  async completeOrder(env, orderId, refId) {
    ...
    try {
      ...
      await env.DB.batch([
        ...
        env.DB.prepare('UPDATE usage SET credits = credits + ?, paid_total = paid_total + ?, updated_at = ? WHERE user_id = ?')
          .bind(order.credits, order.credits, new Date().toISOString(), order.user_id),
        env.DB.prepare(
