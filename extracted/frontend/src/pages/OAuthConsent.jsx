  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // Gate on the server's auth result (data.authenticated), not a client-side
        // SDK check: a cookie-only session (platform login/SSO, or a private app
        // with a stale localStorage token) would read as signed-out on the bearer
        // path and redirect — even though /consent-info just authenticated this
        // same request via its cookie fallback. data.authenticated keeps the
        // redirect decision in agreement with what the server returned.
