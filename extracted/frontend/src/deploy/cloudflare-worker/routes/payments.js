 ...
 *
 *   POST /payments/create          — create payment (returns redirect URL)
 *   GET  /payments/zarinpal/callback — ZarinPal callback (verify + complete)
 *   GET  /payments/status/:orderId  — check order status
 *
 * Payments are NEVER confirmed without gateway verification.
 * On success: verify → update order → grant credits (atomic).
 */
...
export async function handlePaymentRoutes(request, env, url, user) {
  ...
  if (path === '/payments/create' && method === 'POST') {
    ...
    if (!amount || !plan || !credits) {
      return errorResponse(ErrorCodes.INVALID_INPUT, 400, 'مبلغ، طرح و اعتبار الزامی است.');
    }

    if (!payments.isAvailable(env)) {
      return errorResponse(ErrorCodes.NO_PAYMENT_PROVIDER);
    }

    // Create order record
    const orderResult = await payments.createOrder(env, {
      userId: user.id, amount, plan, credits, description: description || '',
    });
    ...
    const payResult = await payments.createPayment(env, {
      amount, description: description || 'Homa AI - ' + plan,
    ...
    try {
      await env.DB.prepare('UPDATE orders SET authority = ? WHERE id = ?')
        .bind(payResult.authority, orderResult.id).run();
    } catch {}
  ...
  if (path === '/payments/zarinpal/callback' && method === 'GET') {
    ...
    if (!authority || !orderId) {
      return errorResponse(ErrorCodes.INVALID_INPUT, 400, 'پارامترهای callback نامعتبر.');
    }

    // ZarinPal: status OK = success
    ...
    try {
      ...
      // Complete order: update status + grant credits (atomic)
      const completeResult = await payments.completeOrder(env, orderId, verifyResult.ref_id);
      if (completeResult.error) {
        return errorResponse(ErrorCodes.INTERNAL_ERROR, 500, completeResult.error);
      }
