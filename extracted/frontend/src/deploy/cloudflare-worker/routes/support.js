 ...
 *   PATCH  /support/tickets/:id          — update ticket (close, etc.)
 *
 * Email notifications are sent via the email abstraction (Resend).
 * If no email provider is configured, tickets are still created but
 * no email is sent (NO_EMAIL_PROVIDER is logged, not returned to user).
 */
...
export async function handleSupportRoutes(request, env, url, user) {
  ...
  if (path === '/support/tickets' && method === 'POST') {
    ...
    if (!body.subject || !body.message) {
      return errorResponse(ErrorCodes.INVALID_INPUT, 400, 'موضوع و پیام الزامی است.');
    }
    ...
    // Send notification email to admin (best-effort, don't fail ticket creation)
    if (email.isAvailable(env)) {
      email.sendSupportNotification(env, {
        to: env.ADMIN_EMAIL || 'support@homa.ai',
        ticket: { subject: body.subject, message: body.message, status: 'open' },
      }).catch(() => {});
    }
  ...
  if (replyMatch && method === 'POST') {
    ...
    try {
      ...
      await env.DB.prepare(
        'UPDATE support_tickets SET replies = ?, status = ?, updated_at = ? WHERE id = ?'
  ...
  // PATCH /support/tickets/:id — update (close, etc.)
  if (ticketMatch && method === 'PATCH') {
    ...
    try {
      await env.DB.prepare(`UPDATE support_tickets SET ${sets}, updated_at = ? WHERE id = ? AND user_id = ?`)
        .bind(...vals, new Date().toISOString(), ticketId, user.id).run();
  ...
  // ===== Admin reply route =====
  // POST /support/tickets/:id/admin-reply
  ...
  if (adminReplyMatch && method === 'POST') {
    ...
    try {
      ...
      replies.push({
        content: body.content,
        by_admin: true,
        author_name: body.author_name || 'پشتیبانی هُما',
        created_at: new Date().toISOString(),
      });
      ...
      if (email.isAvailable(env) && ticket.user_email) {
        email.send(env, {
          ...
          subject: 'پاسخ به تیکت پشتیبانی — هُما AI',
          html: `<div dir="rtl" style="font-family: sans-serif; padding: 20px;"><h2>پاسخ به تیکت شما</h2><p><strong>موضوع:</strong> ${ticket.subject}</p><div style="padding: 12px; background: #f5f5f5; border-radius: 8px;">${body.content}</div></div>`,
        }).catch(() => {});
      }
