 ...
 *
 *   GET    /admin/dashboard       — stats (users, orders, credits, tickets)
 *   GET    /admin/users            — list users
 *   PATCH  /admin/users/:id        — update user (role, etc.)
 *   GET    /admin/orders           — list all orders
 *   GET    /admin/tickets          — list all tickets
 *   PATCH  /admin/tickets/:id      — update ticket (status, priority)
 *   GET    /admin/provider-health  — check provider availability
 *   GET    /admin/credits/transactions — credit transaction ledger
 */
...
export async function handleAdminRoutes(request, env, url) {
  ...
  if (path === '/admin/dashboard' && method === 'GET') {
    try {
      ...
      return successResponse({
        users: userCount?.c || 0,
        paid_orders: orderCount?.c || 0,
        open_tickets: ticketCount?.c || 0,
        total_credits: totalCredits?.c || 0,
        total_revenue: totalRevenue?.c || 0,
      });
    } catch (e) {
  ...
  if (path === '/admin/users' && method === 'GET') {
    try {
      const { results } = await env.DB.prepare(
        'SELECT id, email, full_name, role, created_at FROM users ORDER BY created_at DESC LIMIT 200'
      ).all();
  ...
  if (userMatch && method === 'PATCH') {
    ...
    try {
      await env.DB.prepare(`UPDATE users SET ${sets}, updated_at = ? WHERE id = ?`)
        .bind(...vals, new Date().toISOString(), userId).run();
  ...
  if (ticketMatch && method === 'PATCH') {
    ...
    const vals = cols.map(c => updates[c]);
    try {
      await env.DB.prepare(`UPDATE support_tickets SET ${sets}, updated_at = ? WHERE id = ?`)
        .bind(...vals, new Date().toISOString(), ticketId).run();
      return successResponse({ updated: true });
    } catch (e) {
      return errorResponse(ErrorCodes.INTERNAL_ERROR, 500, e.message);
    }
  ...
  if (path === '/admin/provider-health' && method === 'GET') {
    ...
    for (const cap of capabilities) {
      health[cap] = {
        available: router.isAvailable(cap, env),
        providers: router.getProviders(cap, env),
      };
    }
  ...
  if (path === '/admin/credits/transactions' && method === 'GET') {
    try {
      const { results } = await env.DB.prepare(
        'SELECT * FROM credit_transactions ORDER BY created_at DESC LIMIT 200'
      ).all();
